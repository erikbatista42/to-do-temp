...
//            try moc.save()
    ...
    func restoreTask(indexPath: IndexPath){

//        let confirmation = UIAlertController(title: nil, message: "Restore this task?", preferredStyle: .actionSheet)
//        let restoreAction = UIAlertAction(title: "Yes", style: .destructive) { [weak self] (_) in
//            guard let self = self else { return }
//            self.historyTableView.beginUpdates()
...
//            self.updateTask()
//        self.completedList[indexPath.row].isComplete = false
        let task = self.completedList.remove(at: indexPath.row)
        print(task.title)
//        self.updateTask()
...
//            self.historyTableView.reloadData()
//            self.moc.delete(task)
//            self.moc.insert(task)
//
//            do {
//                try self.moc.save()
//            } catch {
//                self.completedList.removeLast()
//                print(error.localizedDescription)
//            }
//            do {
//                try self.moc.save()
...
//        let noAction = UIAlertAction(title: "No", style: .cancel) { (_) in
//            print("not going to restore")
//            return
//        }
//        confirmation.addAction(restoreAction)
//        confirmation.addAction(noAction)
//        present(confirmation, animated: true, completion: nil)
    }
}
...
extension TaskHistoryViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, leadingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let restoreTask = UIContextualAction(style: .normal, title: .empty) {  (_, _, _) in
...
//            tableView.reloadData()
        ...
        restoreTask.backgroundColor = .systemBlue
//        restoreTask.title = Constants.Action.restore
        restoreTask.title = "Restore"
        let swipeActions = UISwipeActionsConfiguration(actions: [restoreTask])
